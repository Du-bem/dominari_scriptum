  ...
  <div class="astral-tracker-container">
    ...
    <div class="tracker-panel">
      <div class="panel-header">
        <h2 class="text-lg font-bold text-white">Astral Body Tracker</h2>
      ...
      <div class="astral-data-card">
        ...
        <div v-else-if="astralData" class="astral-content">
          ...
          <div class="object-header">
            <component :is="getObjectIcon(astralData.name)" class="h-6 w-6 text-blue-400" />
            <h3 class="text-lg font-bold text-white ml-2">{{ astralData.name }}</h3>
          ...
          <div class="time-stamp">
            ...
            <span class="text-gray-300 text-xs">{{ formatTime(astralData.time) }}</span>
          </div>
          ...
          <div class="data-section">
            ...
            <div class="coordinate-grid">
              <div class="coordinate">
                ...
                <span class="coordinate-value">{{ formatCoordinate(astralData.position[0]) }}</span>
              </div>
              <div class="coordinate">
                <span class="coordinate-label">Y</span>
                <span class="coordinate-value">{{ formatCoordinate(astralData.position[1]) }}</span>
              </div>
              <div class="coordinate">
                <span class="coordinate-label">Z</span>
                <span class="coordinate-value">{{ formatCoordinate(astralData.position[2]) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="data-section">
            ...
            <div class="coordinate-grid">
              <div class="coordinate">
                <span class="coordinate-label">X</span>
                <span class="coordinate-value">{{ formatVelocity(astralData.velocity[0]) }}</span>
              </div>
              <div class="coordinate">
                <span class="coordinate-label">Y</span>
                <span class="coordinate-value">{{ formatVelocity(astralData.velocity[1]) }}</span>
              </div>
              <div class="coordinate">
                <span class="coordinate-label">Z</span>
                <span class="coordinate-value">{{ formatVelocity(astralData.velocity[2]) }}</span>
              </div>
            </div>
            <div class="velocity-magnitude">
              <span class="text-xs text-gray-400">Magnitude:</span>
              <span class="text-white text-xs font-semibold ml-1">{{ calculateMagnitude(astralData.velocity) }} m/s</span>
            </div>
          </div>
          
          <!-- Last Updated -->
          <div class="last-updated">
            <span class="text-xs text-gray-500">Updated: {{ new Date().toLocaleTimeString() }}</span>
            <button 
              @click="fetchData" 
...
import { 
  Loader, AlertCircle, Database, Clock, MapPin, Zap, 
  Star, Globe, Activity, Moon, ChevronDown, RefreshCw 
} from 'lucide-vue-next';

// Astral Body Types Dropdown
const dropdownOpen = ref(false);
const bodyTypes = ref(['All Bodies', 'Asteroids', 'Planets', 'Stars', 'Moons', 'Satellites']);
...
const calculateMagnitude = (vector) => {
  ...
  const sumOfSquares = vector.reduce((sum, component) => sum + Math.pow(component, 2), 0);
...
const getObjectIcon = (name) => {
  ...
  if (nameLower.includes('moon') || nameLower.includes('satellite')) return Moon;
...
const fetchData = async () => {
  ...
  try {
    ...
    const type = selectedBodyType.value === 'All Bodies' 
      ? bodyTypes.value[Math.floor(Math.random() * (bodyTypes.value.length - 1)) + 1]
    ...
    astralData.value = {
      name: `${singularType} ${randomId}`,
      time: `${day}/${month}/${year} ${hours}:${minutes}:${seconds}`,
      position: [
        (Math.random() * 200000 - 100000).toFixed(2) * 1,
        (Math.random() * 200000 - 100000).toFixed(2) * 1,
        (Math.random() * 50000 - 25000).toFixed(2) * 1
      ],
      velocity: [
        (Math.random() * 100 - 50).toFixed(2) * 1,
        (Math.random() * 100 - 50).toFixed(2) * 1,
        (Math.random() * 50 - 25).toFixed(2) * 1
      ]
    };
...
.astral-tracker-container {
  ...
  background-image: radial-gradient(circle at 50% 50%, rgba(16, 24, 48, 0.8) 0%, rgba(8, 8, 24, 1) 100%);
  min-height: 100vh;
}
