  ...
  update(deltaTime, allPlanets) {
    ...
    const calculateGravitationalForce = (planet1, planet2) => {
      ...
      const distanceSquared =
        rVector[0] ** 2 + rVector[1] ** 2 + rVector[2] ** 2;
      const distance = Math.sqrt(distanceSquared);

      const forceMagnitude =
        (G * planet1.mass * planet2.mass) / distanceSquared;
    ...
    const derivatives = (position, velocity) => {
      ...
      for (let i = 0; i < allPlanets.length; i++) {
        if (i !== myIndex) {
          // Changed from 'i > myIndex' to 'i !== myIndex'
    ...
    this.pos[0] +=
      ((k1.dPos[0] + 2 * k2.dPos[0] + 2 * k3.dPos[0] + k4.dPos[0]) *
        deltaTime) /
      6;
    this.pos[1] +=
      ((k1.dPos[1] + 2 * k2.dPos[1] + 2 * k3.dPos[1] + k4.dPos[1]) *
        deltaTime) /
      6;
    this.pos[2] +=
      ((k1.dPos[2] + 2 * k2.dPos[2] + 2 * k3.dPos[2] + k4.dPos[2]) *
        deltaTime) /
      6;

    this.vel[0] +=
      ((k1.dVel[0] + 2 * k2.dVel[0] + 2 * k3.dVel[0] + k4.dVel[0]) *
        deltaTime) /
      6;
    this.vel[1] +=
      ((k1.dVel[1] + 2 * k2.dVel[1] + 2 * k3.dVel[1] + k4.dVel[1]) *
        deltaTime) /
      6;
    this.vel[2] +=
      ((k1.dVel[2] + 2 * k2.dVel[2] + 2 * k3.dVel[2] + k4.dVel[2]) *
        deltaTime) /
      6;
    const planets = usePlanetsStore();
    if (this.name === planets.data.name) {
      planets.setDataPosAndVel(this.pos, this.vel);
    }
  }
